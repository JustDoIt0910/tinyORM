...
namespace orm {
    //变量名称转数据库中的名称，驼峰变下划线
    std::string cppToDb(const std::string& name)
    {
        std::string res;
        if(name.length() == 0)
            return res;
    ...
    std::string dbToCpp(const std::string& name, bool cap_ini = false)
    {
        std::string res;
        if(name.length() == 0)
            return res;
    ...
    bool isSameDbType(const std::string& type1, const std::string& type2)
    {
    ...
    std::string cppTypeToDbType(const std::string& cppType)
    {
        Config* conf = Config::getInstance();
        try {
    ...
    std::string dbTypeToCppType(const std::string& dbType)
    {
