...
//        |Header(model3)|ColumnField1|ColumnField2|
        ...
        static void saveModel(FieldVec& fields, Indices& indices,
                               const std::string& db, Header& header);
        ...
        static std::string dataFilePath(const std::string& db);
    ...
    private:
        ...
        static void parseHeader(std::ifstream& ifs, Header& header);
        static void parseColumnField(std::ifstream& ifs, FieldMeta& field);
        static void parseIndexField(std::ifstream& ifs, Index& index);
    };
    ...
    void DataLoader::loadFromDB(FieldVec& fields, Indices& indices, NameToIndex& nameToIndex,
                                const std::string& db, const std::string& tableName,
                                SqlConn &conn)
    {
    ...
    void DataLoader::saveModel(std::vector<FieldMeta>& fields,
                               std::unordered_map<std::string, Index> &indices, const std::string &db,
                               Header& header)
    {
        std::ofstream ofs(dataFilePath(db), std::ios::binary | std::ios::app);
    ...
    void DataLoader::loadFromFile(const std::string& db, const std::string& className, std::string& tableName,
                             FieldVec& fields, Indices& indices, NameToIndex& nameToIndex)
    {
        std::ifstream ifs(dataFilePath(db), ios::binary);
        if(!ifs.is_open())
            return;
    ...
    std::string DataLoader::dataFilePath(const std::string& db)
    {
        char buf[50];
        ...
        std::string filename = "models/model_" + db + ".dat";
        return path + filename;
    }

    bool DataLoader::parseMarker(std::ifstream &ifs, uchar* marker)
    {
    ...
    void DataLoader::parseHeader(std::ifstream &ifs, orm::DataLoader::Header &header)
    {
    ...
    void DataLoader::parseColumnField(std::ifstream &ifs, FieldMeta& fieldMeta)
    {
        char len;
        ifs.read(&len, 1);
    ...
    void DataLoader::parseIndexField(std::ifstream &ifs, orm::Index &index)
    {
        char type;
        ifs.read(&type, 1);
