...
namespace orm {
    ...
    FieldMeta::FieldMeta(): isPk(false), autoInc(false), hasIndex(false), notNull(false), ignore(false){}
    ...
    std::string FieldMeta::getColumnName() const {
        return columnName.value_or(cppToDb(fieldName));
    }

    //如果没有显式定义列类型，使用rules.json中定义的默认类型
    std::string FieldMeta::getColumnType() const
    {
